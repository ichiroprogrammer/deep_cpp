...
{
    auto fp0 = Nstd::FixedPoint<int, 7>{0xff, 0x7f};
...
{
    using FP8 = Nstd::FixedPoint<int16_t, 8>;
...
{
    using FP8 = Nstd::FixedPoint<int32_t, 8>;  // 基本型int32_t、小数点8ビット
...
{
    using FP8 = Nstd::FixedPoint<int32_t, 8>;
    auto fp0  = FP8{};
...
TEST(FixedPoint, int_8)
{
    ...
    // clang-format off
    auto fp1 = Nstd::FixedPoint<int, 8>{0, 1};
    auto fp2 = fp0 + fp1;                     ASSERT_EQ((FP8{0xff, 0x80}), fp2);

    fp2 += Nstd::FixedPoint<int, 8>{0, 0x80};       ASSERT_EQ((FP8{0x100, 0}), fp2);
    fp2 += Nstd::FixedPoint<int, 8>{0xffff01, 0x0}; ASSERT_EQ(1, fp2); // overflow
    fp2 += Nstd::FixedPoint<int, 8>{-1, 0x0};       ASSERT_EQ(0, fp2);

    fp2 += Nstd::FixedPoint<int, 8>{-1, 0x0};       ASSERT_EQ(-1, fp2);
...
{
    auto fp0 = Nstd::FixedPoint<uint32_t, 8>{};
...
TEST(FixedPoint, fixed_point_literals)
{
    // @@@ sample begin 0:1
