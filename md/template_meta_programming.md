...
[パラメータパック](---)を使って実装するべきである。

### パラメータパックを使用したテクニック
[パラメータパック](---)を使用するには独特なテクニックが必要となるため、まずは単純な例から説明する。
...
* using宣言によりstd::vectorのすべてのコンストラクタをApp::Ints_tに導入
  (「[特殊メンバ関数|継承コンストラクタ](---)」参照)

としているため、エイリアスで宣言されたInts_tと等価である。
C++03では、[特殊メンバ関数|継承コンストラクタ](---)が使えなかったため、
上記のような構造体を定義するためには、
...
これを初期化するためには初期化子リスト
(「[特殊メンバ関数|初期化子リストコンストラクタ](---)」、
「[オブジェクト生成と初期化|一様初期化](---)」参照)を用いるのが良い。
...
```cpp
    // @@@ essential/h/nstd_concepts.h #3:0 begin
```
単体テストは以下のようになる。
...
```cpp
    // @@@ essential/h/nstd_concepts.h #4:0 begin -1
```
...
```cpp
    // @@@ essential/h/nstd_concepts.h #5:0 begin -1
```
...
の戻り型が配列型の[expression|rvalue](---)である"int (&&) [3]"となり、
...
```cpp
    // @@@ essential/h/nstd_concepts.h #0:0 begin
```
```cpp
...
```cpp
    // @@@ essential/h/nstd_concepts.h #0:1 begin
```
```cpp
...
```cpp
    // @@@ essential/h/nstd_concepts.h #1:0 begin
```
...
```cpp
    // @@@ essential/h/nstd_concepts.h #2:0 begin
```
```cpp
...
[型システムと算術の基礎|浮動小数点型](---)を頻繁に使用するソフトウェアの開発を行うに場合、
...
[forwardingリファレンス](---)を使って下記のように書き直した。
...
この原因は、
「関数が受け取った[expression|rvalue](---)リファレンスは、
...
(T const&&はconstな[expression|rvalue](---)リファレンスである)、
forwardingリファレンスが[lvalue](---)リファレンスであった場合は、
...
なお、forwardingリファレンスは、[リファレンスcollapsing](---)の一機能としても理解できる。
...
```cpp
    // @@@ essential/h/nstd_type2str.h #0:0 begin
    // @@@ essential/h/nstd_type2str.h #0:1 begin
    // @@@ essential/h/nstd_type2str.h #0:2 begin
```
...
```cpp
    // @@@ essential/h/nstd_type2str.h #0:1 begin -1
```
...
```cpp
    // @@@ essential/h/scoped_guard.h #0:3 begin
```
...
* forwardingリファレンスを持つ関数テンプレートをオーバーロードしない。
  「[forwardingリファレンスとstd::forward](---)」で述べたように、
