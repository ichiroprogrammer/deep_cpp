...
ような場合に使用する。
そのため[オープン・クローズドの原則(OCP)](solid.md#SS_1_2)の実装方法としても有用である。
...
[特殊メンバ関数](term_explanation.md#SS_5_2_1)では機能が不十分であることが多い。
...
* copy代入演算子には、[エクセプション安全性の保証](term_explanation.md#SS_5_7)がない。
...
これにより[エクセプション安全性の保証](term_explanation.md#SS_5_7)を持つ4関数をコードクローンすることなく実装できる。
...
一方で、「[Accessor](design_pattern.md#SS_2_4)」で紹介したsetterは、クラスのimmutabilityを下げる。
...
## Clone(仮想コンストラクタ) <a id="SS_2_7"></a>
オブジェクトコピーによる[スライシング](term_explanation.md#SS_5_3_3)を回避するためのイデオムである。
...
## NVI(non virtual interface) <a id="SS_2_8"></a>
...
## RAII(scoped guard) <a id="SS_2_9"></a>
...
## DI(dependency injection) <a id="SS_2_11"></a>
メンバ関数内でクラスDependedのオブジェクトを直接、生成する
(もしくは[Singleton](design_pattern.md#SS_2_12)オブジェクトや静的オブジェクト(std::coutやstd::cin等)に直接アクセスする)
クラスNotDIがあるとする。
...
(「[Named Constructor](design_pattern.md#SS_2_17)」参照)と呼ばれる。


## State <a id="SS_2_13"></a>
Stateは、オブジェクトの状態と、それに伴う振る舞いを分離して記述するためのパターンである。
これにより状態の追加、削減、変更に伴う修正範囲が限定される
(「[オープン・クローズドの原則(OCP)](solid.md#SS_1_2)」参照)。
...
この性質は、[Factory](design_pattern.md#SS_2_16)や[Named Constructor](design_pattern.md#SS_2_17)の実装には不可欠であるが、
...
DI(「[DI(dependency injection)](design_pattern.md#SS_2_11)」参照)と組み合わせて使われることが多い。

「[Templateメソッド](design_pattern.md#SS_2_15)」の例にFactoryを適用したソースコードを下記する。
...
## Named Constructor <a id="SS_2_17"></a>
Named Connstructorは、[Singleton](design_pattern.md#SS_2_12)のようなオブジェクトを複数、生成するためのパターンである。
...
次に示したのは「[Factory](design_pattern.md#SS_2_16)」の例にこのパターンを適応したコードである。
...
その属性の追加に対して「[オープン・クローズドの原則(OCP)](solid.md#SS_1_2)」に対応した構造となった。
...
あるいは、[DI(dependency injection)](design_pattern.md#SS_2_11)と組み合わせて、下記のような改善も有用である。
...
SRP(「[単一責任の原則(SRP)](solid.md#SS_1_1)」)に反するため、
...
これを改めるためには、「[CRTP(curiously recurring template pattern)](design_pattern.md#SS_2_21)」が必要になる。

次に示すソースコードはVisitorとは関係がないが、
FileEntityVisitorから派生するクラスを下記クラス図が示すように改善することで、
単体テストが容易になる例である(「[DI(dependency injection)](design_pattern.md#SS_2_11)」参照)。
...
## CRTP(curiously recurring template pattern) <a id="SS_2_21"></a>
CRTPとは、
...
このパターンを用いて、「[Visitor](design_pattern.md#SS_2_20)」のFileEntityの3つの派生クラスが持つコードクローン
...
GUIアプリケーションを[MVC](design_pattern.md#SS_2_23)で実装する場合のModelがSubjectであり、
ViewがObserverNである。
...
ViewはModelの[Observer](design_pattern.md#SS_2_22)であるため、ModelはViewへ依存しない。
...
## Cでのクラス表現 <a id="SS_2_24"></a>
このドキュメントは、C++でのソフトウェア開発を前提としているため、
ここで示したコードもC++で書いているが、

* 何らかの事情でCを使わざるを得ないプログラマがデザインパターンを使用できるようにする
* クラスの理解が曖昧なC++プログラマの理解を助ける(「[クラスのレイアウト](term_explanation.md#SS_5_2_11)」参照)
