...
等の問題(「[malloc/freeの問題点](dynamic_memory_allocation.md#SS_4_1)」参照)を持っている。
...
(この実装は長いため、全体は巻末の"[example/dynamic_memory_allocation/malloc_ut.cpp](sample_code.md#SS_6_1_1)"に掲載する)。
...
[フリースタンディング環境](term_explanation.md#SS_5_10_20)では、sbrkのようなシステムコールは存在しないため、
...
## グローバルnew/deleteのオーバーロード <a id="SS_4_2"></a>
...
「[グローバルnew/deleteのオーバーロードの実装](dynamic_memory_allocation.md#SS_4_2_2)」で示すように、
このメモリプールは管理する固定長のメモリブロックのサイズごとに複数必要になる一方で、
これらを統合的に扱う必要も出てくる。

そのため、固定長のメモリプールは、

* 複数個のメモリプールを統合的に扱う[インターフェースクラス](term_explanation.md#SS_5_1_13)MPool
* MPoolを基底クラスとし、固定長メモリブロックを管理するクラステンプレートMPoolFixed

によって実装することにする。

まずは、MPoolを下記に示す
(「[ファイル位置を静的に保持したエクセプションクラスの開発](template_meta_programming.md#SS_3_6_7_4)」参照)。
...
### グローバルnew/deleteのオーバーロードの実装 <a id="SS_4_2_2"></a>
[固定長メモリプール](dynamic_memory_allocation.md#SS_4_2_1)を使用したoperator newのオーバーロードの実装例を以下に示す。
...
[一様初期化](term_explanation.md#SS_5_2_7)で行った方が良いが、
...
gen_mpool<>については、「[プレースメントnew](dynamic_memory_allocation.md#SS_4_2_3)」で説明する。
...
高速に動作するが、malloc/freeの実装(「[malloc/freeの問題点](dynamic_memory_allocation.md#SS_4_1)」参照)で使用したHeader_t
を導入することでこの実行コストはほとんど排除できる。
そのトレードオフとしてメモリコストが増えるため、ここでは例示した仕様にした。


### プレースメントnew <a id="SS_4_2_3"></a>
「[グローバルnew/deleteのオーバーロードの実装](dynamic_memory_allocation.md#SS_4_2_2)」で使用したgen_mpool<>は、
下記のように定義されている。
...
## クラスnew/deleteのオーバーロード <a id="SS_4_3"></a>
「[グローバルnew/deleteのオーバーロードの実装](dynamic_memory_allocation.md#SS_4_2_2)」で示したコードのロックを、
...
* その他のクラスに対しては、[Singleton](design_pattern.md#SS_2_12)や[Named Constructor](design_pattern.md#SS_2_17)と同様な静的な方法を用いる
...
メモリ管理に「[固定長メモリプール](dynamic_memory_allocation.md#SS_4_2_1)」で示したMPoolFixedを利用した実装例を以下に示す。
...
これを避けるためには、
[CRTP(curiously recurring template pattern)](design_pattern.md#SS_2_21)
...
「[グローバルnew/deleteのオーバーロード](dynamic_memory_allocation.md#SS_4_2)」に比べ、優れているように見えてしまうかもしれないが、
...
(「[可変長メモリプール](dynamic_memory_allocation.md#SS_4_4_2)」参照)を使う。

### 可変長メモリプール <a id="SS_4_4_2"></a>
可変長メモリプールを生成するMPoolVariableの実装は下記のようになる
(全体は巻末の"[example/dynamic_memory_allocation/mpool_variable.h](sample_code.md#SS_6_1_2)"に掲載する)。
...
### デバッグ用イテレータ <a id="SS_4_4_3"></a>
[可変長メモリプール](dynamic_memory_allocation.md#SS_4_4_2)を使用すると、
...
「[グローバルnew/deleteのオーバーロードの実装](dynamic_memory_allocation.md#SS_4_2_2)」でも述べたように、
デバッグ用入出力機能からこのような出力を得られるようにしておくべきである。


### エクセプション処理機構の変更 <a id="SS_4_4_4"></a>
...
コンパイラに[g++](term_explanation.md#SS_5_9_1)や[clang++](term_explanation.md#SS_5_9_2)を使っている場合、
