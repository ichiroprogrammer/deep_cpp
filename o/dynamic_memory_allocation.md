...
本章で扱うダイナミックメモリアロケーション([ヒープ](cpp_idioms.md#SS_8_14_1)の使用)とは、new/delete、malloc/free
によるメモリ確保/解放のことである。
...
* 「[グローバルnew/deleteのオーバーロード](dynamic_memory_allocation.md#SS_5_4_1)」で述べたようなnewを実装する。
* グローバルnewを使用せず、 「[クラスnew/deleteのオーバーロード](dynamic_memory_allocation.md#SS_5_4_3)」で述べたようなクラス毎のnewを実装する。
* malloc/newを用いてエクセプション送出を行っているツールチェーンを使用している場合、
 「[エクセプション処理機構の変更](dynamic_memory_allocation.md#SS_5_3_2)」で述べたような方法でリアルタイム制を確保するか、
  エクセプションを使用しない。
...
[malloc/freeの問題点](dynamic_memory_allocation.md#SS_5_1)  
[メモリプール](dynamic_memory_allocation.md#SS_5_2)  
&emsp;[固定長メモリプール](dynamic_memory_allocation.md#SS_5_2_1)  
&emsp;[可変長メモリプール](dynamic_memory_allocation.md#SS_5_2_2)  

[メモリプールのエクセプション](dynamic_memory_allocation.md#SS_5_3)  
&emsp;[MPoolBadAlloc](dynamic_memory_allocation.md#SS_5_3_1)  
&emsp;[エクセプション処理機構の変更](dynamic_memory_allocation.md#SS_5_3_2)  

[new/deleteのオーバーロード](dynamic_memory_allocation.md#SS_5_4)  
&emsp;[グローバルnew/deleteのオーバーロード](dynamic_memory_allocation.md#SS_5_4_1)  
&emsp;[デバッグ用イテレータ](dynamic_memory_allocation.md#SS_5_4_2)  
&emsp;[クラスnew/deleteのオーバーロード](dynamic_memory_allocation.md#SS_5_4_3)  
&emsp;[new/deleteのオーバーロードのまとめ](dynamic_memory_allocation.md#SS_5_4_4)  

[標準ライブラリのコンテナ用アロケータ](dynamic_memory_allocation.md#SS_5_5)  
&emsp;[デバッグ用イテレータ](dynamic_memory_allocation.md#SS_5_5_1)  
...
このアクセスの競合は[スピンロック](cpp_idioms.md#SS_8_14_6)(SpinLock)によって回避される。
...
[フリースタンディング環境](cpp_idioms.md#SS_8_14_9)では、sbrkのようなシステムコールは存在しないため、
...
[malloc/freeの問題点](dynamic_memory_allocation.md#SS_5_1)の問題を回避するすために２種類のメモリプールの実装を示す。

* リアルタイム性が保証されれた[固定長メモリプール](dynamic_memory_allocation.md#SS_5_2_1)
* フラグメントの状態を確認することができる[可変長メモリプール](dynamic_memory_allocation.md#SS_5_2_2)
...
* 複数個のメモリプールを統合的に扱う[インターフェースクラス](core_lang_spec.md#SS_6_4_11)MPool
* MPoolを基底クラスとし、固定長メモリブロックを管理するクラステンプレートMPoolFixed

によって実装することにする。

まずは、MPoolを下記に示す。なお、throwするオブジェクトの型は[MPoolBadAlloc](dynamic_memory_allocation.md#SS_5_3_1)を使用している。
...
なお、この実装ではmalloc/freeと同様に使用制限の少ない[スピンロック](cpp_idioms.md#SS_8_14_6)(SpinLock)を使用したが、
...
- エクセプション用の型[MPoolBadAlloc](dynamic_memory_allocation.md#SS_5_3_1)の開発
- [エクセプション処理機構の変更](dynamic_memory_allocation.md#SS_5_3_2)

を通じて、メモリプールのエクセプション機構を紹介する。

### MPoolBadAlloc <a id="SS_5_3_1"></a>
MPoolBadAllocは下記のように定義されたクラスであり、
「[ファイル位置を静的に保持したエクセプションクラスの開発](template_meta_programming.md#SS_4_7_6_4)」
...
コンパイラに[g++](cpp_idioms.md#SS_8_15_1)や[clang++](cpp_idioms.md#SS_8_15_2)を使っている場合、
...
* [グローバルnew/deleteのオーバーロード](dynamic_memory_allocation.md#SS_5_4_1)
* [クラスnew/deleteのオーバーロード](dynamic_memory_allocation.md#SS_5_4_3)



### グローバルnew/deleteのオーバーロード <a id="SS_5_4_1"></a>
[固定長メモリプール](dynamic_memory_allocation.md#SS_5_2_1)を使用した`operator new`のオーバーロードの実装例を以下に示す。
...
[一様初期化](core_lang_spec.md#SS_6_6_6)で行った方が良いが、
...
[プレースメントnew](core_lang_spec.md#SS_6_6_9)によりMPoolオブジェクトを生成する下記の関数テンプレートである。
...
[グローバルnew/deleteのオーバーロード](dynamic_memory_allocation.md#SS_5_4_1)で示したグローバルnew/deleteの実装は、適切なメモリの量を調整する必要がある。
...
「[グローバルnew/deleteのオーバーロード](dynamic_memory_allocation.md#SS_5_4_1)」で示したコードのロックを、
...
* その他のクラスに対しては、[Singleton](design_pattern.md#SS_3_1_1)や[Named Constructor](design_pattern.md#SS_3_1_2)と同様な静的な方法を用いる

とし、グローバルnewを使用しないことが、より良いメモリ使用方法となり得る。

グローバルnewを使わずに動的にオブジェクトを生成するためには、

* [プレースメントnew](core_lang_spec.md#SS_6_6_9)を使う
...
これを避けるためには、
[CRTP(curiously recurring template pattern)](cpp_idioms.md#SS_8_1_5)
...
「[グローバルnew/deleteのオーバーロード](dynamic_memory_allocation.md#SS_5_4_1)」に比べ、優れているように見えてしまうかもしれないが、
...
1. リアルタイムパスでのオブジェクトの生成/解放を行う必要があるクラスのnew/deleteのオーバーロードを
  [固定長メモリプール](dynamic_memory_allocation.md#SS_5_2_1)により実装する。
2. グローバルnew/deleteのオーバーロードを[可変長メモリプール](dynamic_memory_allocation.md#SS_5_2_2)により実装する。
...
(「[可変長メモリプール](dynamic_memory_allocation.md#SS_5_2_2)」参照)を使う。

C++14の環境で標準ライブラリのアロケータを置き換える場合は、上記のような方法で実現できるが、
C++17以降では、[std::pmr::polymorphic_allocator](stdlib_and_concepts.md#SS_7_7_2)を使用するべきである。

### デバッグ用イテレータ <a id="SS_5_5_1"></a>
[可変長メモリプール](dynamic_memory_allocation.md#SS_5_2_2)を使用すると、
...
「[グローバルnew/deleteのオーバーロード](dynamic_memory_allocation.md#SS_5_4_1)」でも述べたように、
