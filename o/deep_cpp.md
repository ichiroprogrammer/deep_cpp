...
## RAII(scoped guard) <a id="SS_2_1"></a>
## Polymorphic Memory Resource(pmr) <a id="SS_2_2"></a>
## Visitor <a id="SS_2_3"></a>
## パラメータパック <a id="SS_2_4"></a>
## Accessor <a id="SS_2_5"></a>
## Immutable <a id="SS_2_6"></a>

## SOLID <a id="SS_2_7"></a>
### 単一責任の原則(SRP) <a id="SS_2_7_1"></a>
### オープン・クローズドの原則(OCP) <a id="SS_2_7_2"></a>
### リスコフの置換原則(LSP) <a id="SS_2_7_3"></a>
...
&emsp;&emsp;&emsp;&emsp; [lvalueからの代入](#SS_3_15_2_1)  
&emsp;&emsp;&emsp;&emsp; [rvalueからの代入](#SS_3_15_2_2)  
&emsp;&emsp;&emsp;&emsp; [std::move(lvalue)からの代入](#SS_3_15_2_3)  
...
&emsp;&emsp;&emsp; [ハンドル](#SS_3_21_1)  
&emsp;&emsp;&emsp; [フリースタンディング環境](#SS_3_21_2)  
&emsp;&emsp;&emsp; [サイクロマティック複雑度](#SS_3_21_3)  
&emsp;&emsp;&emsp; [凝集度](#SS_3_21_4)  
&emsp;&emsp;&emsp;&emsp; [凝集度の欠如](#SS_3_21_4_1)  
&emsp;&emsp;&emsp;&emsp; [LCOMの評価基準](#SS_3_21_4_2)  

&emsp;&emsp;&emsp; [Spurious Wakeup](#SS_3_21_5)  
&emsp;&emsp;&emsp; [副作用](#SS_3_21_6)  
&emsp;&emsp;&emsp; [is-a](#SS_3_21_7)  
&emsp;&emsp;&emsp; [has-a](#SS_3_21_8)  
&emsp;&emsp;&emsp; [is-implemented-in-terms-of](#SS_3_21_9)  
&emsp;&emsp;&emsp;&emsp; [public継承によるis-implemented-in-terms-of](#SS_3_21_9_1)  
&emsp;&emsp;&emsp;&emsp; [private継承によるis-implemented-in-terms-of](#SS_3_21_9_2)  
&emsp;&emsp;&emsp;&emsp; [コンポジションによる(has-a)is-implemented-in-terms-of](#SS_3_21_9_3)  
...
* リソース管理を外部([RAII(scoped guard)](#SS_2_1)クラス)に任せられる場合: ゼロの法則を採用し、特殊メンバ関数を明示的に定義しない。
...
リソース管理を専門とするクラス
(例: 標準ライブラリの[RAII(scoped guard)](#SS_2_1)クラス)に任せる設計ガイドラインを指す。
...
X、Yオブジェクトへの[ハンドル](#SS_3_21_1)を完全に失った状態であり、X、Yオブジェクトを解放する手段はない。
...
その`shared_ptr`インスタンスとの関連のみを保持するのため、[オブジェクトの循環所有](#SS_3_5_7_3)の問題を解決できる。
...
下記に示すように、[Polymorphic Memory Resource(pmr)](#SS_2_2)のpool_resourceの初期化には、
...
std::variantとstd::visit([Visitor](#SS_2_3)パターンの実装の一種)を組み合わせた場合の使用例を以下に示す。
...
[name lookup](#SS_3_10_2)の対象外になる」現象一般を指す通称である
...
この構文は[パラメータパック](#SS_2_4)の展開においても有用な場合がある。
...
[rvalueリファレンス](#SS_3_15_2)(`T&&`型の変数)はlvalueである。
一方、その初期化に使われる式(例えばstd::move(x))は[xvalue](#SS_3_14_1_3)である。
...
* prvalue(もしくはrvalue)は、
    * アドレス演算子(&)のオペランドになれない。
    * 非constな[lvalueリファレンス](#SS_3_15_1)ではバインドできないが、
      constな[lvalueリファレンス](#SS_3_15_1)や[rvalueリファレンス](#SS_3_15_2)でバインドできる。
  

つまり、prvalueとはいわゆるテンポラリオブジェクトのことである(下記の`std::string{}`で作られるようなオブジェクト)。
多くの場合、prvalueはテンポラリオブジェクトを生成するが、
C++17以降は[RVO(Return Value Optimization)](#SS_3_19_13)により、
...
* 「テンポラリオブジェクト([rvalue](#SS_3_14_1_2))」をバインドできるリファレンス
...
* [lvalueからの代入](#SS_3_15_2_1)
* [rvalueからの代入](#SS_3_15_2_2)
* [std::move(lvalue)からの代入](#SS_3_15_2_3)



#### lvalueからの代入 <a id="SS_3_15_2_1"></a>
下記コードにより「[lvalue](#SS_3_14_1_1)からの代入」を説明する。
...
#### rvalueからの代入 <a id="SS_3_15_2_2"></a>
下記コードにより「[rvalue](#SS_3_14_1_2)からの代入」を説明する。
...
#### std::move(lvalue)からの代入 <a id="SS_3_15_2_3"></a>
...
* 行１  
  「[lvalueからの代入](#SS_3_15_2_1)」の行１と同じである。

* 行２  
  「[lvalueからの代入](#SS_3_15_2_1)」の行２と同じである。

* 行３  
  std::moveは単にrvalueリファレンスへのキャストを行うだけであり、ランタイム時の処理コストは発生しない。
  この例の場合、std::stringがmoveコンストラクタ／move代入演算子を提供しているため、
  下記図のようなバッファの所有が移し替えられるだけである(この代入もmove代入と呼ぶ)。
  この動作は「[rvalueからの代入](#SS_3_15_2_2)の行２の左辺」と同じであり、同様に速度が向上するが、その副作用として、
  str1への代入以外のアクセスは[未規定動作](#SS_3_19_8)であるため、避けるべきである
...
下記GetString0()のような関数が返すオブジェクトの内部メンバに対する[ハンドル](#SS_3_21_1)は、
...
* クラスや関数がその名前から想起できる責務を持っている
    * 「[単一責任の原則(SRP)](#SS_2_7_1)」を満たしている
    * [Accessor](#SS_2_5)を実装する関数の名前は、GetXxxやSetXxxになっている
...
下記に示す通り、このコードは、
[オープン・クローズドの原則(OCP)](#SS_2_7_2)にも対応した柔軟な構造を実現している。
...
### ハンドル <a id="SS_3_21_1"></a>
CやC++の文脈でのハンドルとは、ポインタかリファレンスを指す。

### フリースタンディング環境 <a id="SS_3_21_2"></a>
...
### 凝集度 <a id="SS_3_21_4"></a>
[凝集度](https://ja.wikipedia.org/wiki/%E5%87%9D%E9%9B%86%E5%BA%A6)
とはクラス設計の妥当性を表す尺度の一種であり、
「[凝集度の欠如](#SS_3_21_4_1)(LCOM)」というメトリクスで計測される。

* [凝集度の欠如](#SS_3_21_4_1)メトリクスの値が1に近ければ凝集度は低く、この値が0に近ければ凝集度は高い。
* メンバ変数やメンバ関数が多くなれば、凝集度は低くなりやすい。
* 凝集度は、クラスのメンバがどれだけ一貫した責任を持つかを示す。
* 「[単一責任の原則(SRP)](#SS_2_7_1)」を守ると凝集度は高くなりやすい。
* 「[Accessor](#SS_2_5)」を多用すれば、振る舞いが分散しがちになるため、通常、凝集度は低くなる。
...
(ただし、「[Immutable](#SS_2_6)」の観点からは、QuadraticEquation::Set()がない方が良い)。
...
#### 凝集度の欠如 <a id="SS_3_21_4_1"></a>
[凝集度](#SS_3_21_4)の欠如(Lack of Cohesion in Methods/LCOM)とは、
...
[LCOMの評価基準](#SS_3_21_4_2)に具体的な推奨値を示す。

#### LCOMの評価基準 <a id="SS_3_21_4_2"></a>
...
* `LCOM <= 0.4`  
  クラスが非常に凝集しており、[単一責任の原則(SRP)](#SS_2_7_1)を強く遵守している状態であるため、
  通常、デザインの見直しは不要である。
...
### Spurious Wakeup <a id="SS_3_21_5"></a>
...
bird::flyのオーバーライド関数(penguin::fly)について、[リスコフの置換原則(LSP)](#SS_2_7_3)に反した例を下記する。
...
kyukanchoとstd::stringの関係を[has-a](#SS_3_21_8)の関係と呼ぶ。


### has-a <a id="SS_3_21_8"></a>
「has-a」の関係は、
...
### is-implemented-in-terms-of <a id="SS_3_21_9"></a>
...
自身の機能を提供する場合に使われる。
[has-a](#SS_3_21_8)の関係は、is-implemented-in-terms-of の関係の一種である。

is-implemented-in-terms-ofは下記の手段1-3に示した方法がある。

*手段1.* [public継承によるis-implemented-in-terms-of](#SS_3_21_9_1)  
*手段2.* [private継承によるis-implemented-in-terms-of](#SS_3_21_9_2)  
*手段3.* [コンポジションによる(has-a)is-implemented-in-terms-of](#SS_3_21_9_3)  
...
手段1は[is-a](#SS_3_21_7)に見え、以下に示すような問題も考慮する必要があるため、
可読性、保守性を劣化させる可能性がある。
...
#### public継承によるis-implemented-in-terms-of <a id="SS_3_21_9_1"></a>
...
すでに述べたようにこの方法は、
[private継承によるis-implemented-in-terms-of](#SS_3_21_9_2)や、
[コンポジションによる(has-a)is-implemented-in-terms-of](#SS_3_21_9_3)
と比べコードがシンプルになる。 

#### private継承によるis-implemented-in-terms-of <a id="SS_3_21_9_2"></a>
...
この方法は、[public継承によるis-implemented-in-terms-of](#SS_3_21_9_1)が持つデストラクタ問題は発生せす、
[is-a](#SS_3_21_7)と誤解してしまう問題も発生しない。


#### コンポジションによる(has-a)is-implemented-in-terms-of <a id="SS_3_21_9_3"></a>
