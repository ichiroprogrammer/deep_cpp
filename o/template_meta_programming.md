...
このドキュメントで使用している[g++](cpp_idioms.md#SS_8_12_1)ではこのコードはコンパイルでき、
動作も問題ないように思われるが、[clang++](cpp_idioms.md#SS_8_12_2)では以下のようなエラーが発生し、コンパイルできない。
...
* operator<<(std::ostream& os, App::Ints_t const& ints)
  ...
  (「[割れ窓理論](cpp_idioms.md#SS_8_13_2)」参照)になってしまうかもしれない)。
...
コードクローンや、[車輪の再発明](cpp_idioms.md#SS_8_13_3)による開発効率の低下につながることがある。
...
実際、現在使用中の[g++](cpp_idioms.md#SS_8_12_1)ではこのコードはコンパイルできず、
...
残念なことにこの関数は非標準であるため、
それを使っているNstd::Inner\_::demangleは[g++](cpp_idioms.md#SS_8_12_1)/[clang++](cpp_idioms.md#SS_8_12_2)
でなければコンパイルできないだろう。
